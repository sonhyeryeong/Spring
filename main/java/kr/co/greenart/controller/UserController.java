...
//		}
	...
	// 유효성검사한 case~
	// 6. model안의 User객체를 덮어씌운채로 이리 옴 -> 유효성검사를 UserValidator로 할거임! ->
	// UserValidator에서 이어서..
//		@PostMapping("/user")
...
//		}
	// 위의 방법처럼 Validate하는 클래스 수동으로 안만들고 Hibernate Validator Engine을 써서 어노테이션으로 쉽게
	// 유효성 검사도 가능!
	// hibernate-validator라이브러리 추가하고 -> 유효성 검사할 User클래스에 어노테이션 붙이고 -> 여기서 @Valid먹이면
	// 끝!
	@PostMapping("/user")
	public String submit(@ModelAttribute("userinfo") @Valid User user, BindingResult errors) { // 가진 requestParam으로
																								// User바인딩해서 모델에 넣기
		logger.info("입력정보: " + user.toString());
		if (errors.hasErrors()) {
			return "userForm";
	...
	/*
	 * @GetMapping("/user/list") public String list(Model model) {
	 * model.addAttribute("list", service.list()); return "userlist"; }
	 * 
	 * @PostMapping("/user") public String submit(@ModelAttribute("user") @Valid
	 * User user, BindingResult errors) { logger.info("입력 정보: " + user.toString());
	 * 
	 * // validator.validate(user, errors);
	 * 
	 * if (errors.hasErrors()) { logger.info(errors.toString()); return "userForm";
	 * } service.add(user);
	 * 
	 * return "redirect:/user/list"; } //
	 * 
	 * @ModelAttribute("user") public User user() { return new User("기본값", 1); }
	 * 
	 * @GetMapping("/user") public String userForm(@ModelAttribute("user") User
	 * user) { return "userForm"; }
	 */
